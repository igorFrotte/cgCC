...
struct Uniform{
	...
	Uniform& operator=(const std::initializer_list<vec3>& v){
		glUniform3fv(loc, v.size(), (float*)std::data(v));
		return *this;
	}
};
...
struct GLBuffer : public UintResource{
	...
	template<class Data>
	void data(const Data& V, GLenum usage = GL_STATIC_DRAW){
...
struct GLTexture : public UintResource{
	...
	void load(std::string filename, GLenum target);
	void load(std::string filename){ load(filename, this->target); }
};
